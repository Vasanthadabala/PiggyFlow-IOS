...
final class CloudSyncManager: ObservableObject {
        ...
        do {
            ...
            for change in snapshot.documentChanges {
                ...
                case .added, .modified:
                    ...
                    applyIncomeDetail(data, to: income)
    ...
    /// The optional detail fields the income form captures. Split out so the incremental and
    /// full snapshot paths can't drift apart as fields are added.
    ///
    /// A document written before these fields existed has no key for them, and the readers
    /// return an empty value rather than nil — so remote records simply arrive without detail
    /// instead of failing to decode.
    private func applyIncomeDetail(_ data: [String: Any], to income: Income) {
        let currency = stringValue(from: data["currencyCode"])
        income.currencyCode = currency.isEmpty ? AppConstants.Currency.code : currency
        income.payer = stringValue(from: data["payer"])
        income.paymentMethod = stringValue(from: data["paymentMethod"])
        income.account = stringValue(from: data["account"])
        income.incomeType = stringValue(from: data["incomeType"])
        income.tags = stringArrayValue(from: data["tags"])
    }
